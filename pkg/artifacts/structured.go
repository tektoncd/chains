...
// intoto Statements. They are also used in SBOM signing.
// URI is the resource uri for the target needed iff the target is a material.
// Digest is the target's SHA digest.
// SBOMURI is the resource uri for the target's SBOM .
// SBOMFormat is the format of the target's SBOM.
...
type structuredSignableExtractor struct {
	uriSuffix        string
	digestSuffix     string
	sbomURISuffix    string
	sbomFormatSuffix string
...
func (b *structuredSignableExtractor) extract(ctx context.Context, obj objects.TektonObject) []StructuredSignable {
	...
	suffixes := map[string]func(StructuredSignable, string) StructuredSignable{
		...
		b.sbomURISuffix: func(s StructuredSignable, value string) StructuredSignable {
			s.SBOMURI = value
			return s
		},
		b.sbomFormatSuffix: func(s StructuredSignable, value string) StructuredSignable {
			s.SBOMFormat = value
			return s
		},
	}
