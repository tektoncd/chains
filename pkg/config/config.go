...
type OCIStorageConfig struct {
	Repository string
	Insecure   bool
	// DistributionMethod controls where and how artifacts are attached to images in the
	// registry, and implicitly the payload encoding:
	//   - "legacy" (default): tag-based storage with DSSE-encoded payloads.
	//   - "referrers-api": OCI 1.1 Referrers API with Sigstore protobuf-bundle attestations.
...
const (
	...
	ociDistributionMethodKey   = "storage.oci.distribution-method"
	...
	// OCIDistributionLegacy is the default tag-based distribution method (full backward compatibility).
	// It stores DSSE-encoded payloads under .sig/.att tags.
	OCIDistributionLegacy = "legacy"
	// OCIDistributionReferrersAPI uses the OCI 1.1 Referrers API, reducing tag proliferation.
	// Attestations are stored as Sigstore protobuf bundles.
	OCIDistributionReferrersAPI = "referrers-api"
)
...
// validateOCIDistributionMethod returns an error if the provided distribution method is not
// one of the supported values.
func validateOCIDistributionMethod(method string) error {
	if method == "" {
		return nil // empty defaults to legacy
	}
	switch method {
	case OCIDistributionLegacy, OCIDistributionReferrersAPI:
		return nil
	}
	return fmt.Errorf("invalid storage.oci.distribution-method %q: must be one of %q, %q",
		method, OCIDistributionLegacy, OCIDistributionReferrersAPI)
}
...
func defaultConfig() *Config {
	return &Config{
		...
		Storage: StorageConfigs{OCI: OCIStorageConfig{
			DistributionMethod: OCIDistributionLegacy,
		}, Grafeas: GrafeasConfig{
...
func NewConfigFromMap(data map[string]string) (*Config, error) {
	...
	if err := cm.Parse(data,
		...
		asString(ociDistributionMethodKey, &cfg.Storage.OCI.DistributionMethod),
	...
	// Default to legacy if no distribution method was resolved.
	if cfg.Storage.OCI.DistributionMethod == "" {
		cfg.Storage.OCI.DistributionMethod = OCIDistributionLegacy
	}

	if err := validateOCIDistributionMethod(cfg.Storage.OCI.DistributionMethod); err != nil {
		return nil, err
	}
