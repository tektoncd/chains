...
func TestOTelMetrics(t *testing.T) {
	...
	// The Chains signing counters kept identical Prometheus names across the
	// OpenCensus-to-OTel migration: Knative's sharedmain prepends the component
	// name as the OC domain/namespace, so the old OC names were already
	// watcher_taskrun_sign_created_total, watcher_pipelinerun_sign_created_total,
	// etc. The metrics that were actually renamed are Knative's infrastructure
	// metrics, e.g. the workqueue metrics moved from the watcher_workqueue_*
	// prefix to kn_workqueue_*. Assert the old prefix is gone.
	// TODO: Remove this assertion in a future release once no OC-based release
	// is supported.

	removedPrefixTests := []struct {
		name   string
		prefix string
		errMsg string
	}{
		{"Removed/old_workqueue_prefix_absent", "watcher_workqueue_", "Old OC metric prefix watcher_workqueue_* still present; expected rename to kn_workqueue_* after OTel migration"},
	}
	for _, tt := range removedPrefixTests {
		t.Run(tt.name, func(t *testing.T) {
			for name := range families {
				if strings.HasPrefix(name, tt.prefix) {
					t.Error(tt.errMsg)
